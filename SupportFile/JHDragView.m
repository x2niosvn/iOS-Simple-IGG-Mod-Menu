...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (CGRectGetWidth(frame) <= 0 ||
        ...
        frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 70, 130, 35, 35);
    ...
    if (self) {
       self.layer.borderColor = [[UIColor colorWithRed: 1.0 green: 0.00 blue: 0.00 alpha: 0.50] CGColor];
        //self.layer.borderColor = [[UIColor whColor] CGColor];
       ...
       self.backgroundColor=  [UIColor colorWithRed: 0.00 green: 0.00 blue: 0.00 alpha: 0.20];
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            NSData *imageData = [NSData dataWithContentsOfURL:[NSURL URLWithString:@"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTq-dWv1vixhXikYjDRwDSjgncIQA9no1IpwA&usqp=CAU"]];
